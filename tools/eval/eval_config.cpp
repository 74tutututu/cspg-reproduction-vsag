...
EvalConfig::Load(argparse::ArgumentParser& parser) {
    ...
    config.build_base_limit = parser.get<uint64_t>("--build-base-limit");
...
EvalConfig::Load(YAML::Node& yaml_node, const eval_job& global_options) {
    ...
    check_and_get_value<uint64_t>(yaml_node, "build_base_limit", config.build_base_limit);
    check_and_get_value<uint64_t>(yaml_node, "search_query_count", config.search_query_count);
    check_and_get_value<uint64_t>(yaml_node, "search_query_limit", config.search_query_limit);
    check_and_get_value<bool>(yaml_node, "enable_cspg_stats", config.enable_cspg_stats);
...
EvalConfig::CheckKeyAndType(YAML::Node& yaml_node) {
    ...
    check_and_get_value<uint64_t>(yaml_node, "build_base_limit");
    check_and_get_value<uint64_t>(yaml_node, "search_query_count");
    check_and_get_value<uint64_t>(yaml_node, "search_query_limit");
    check_and_get_value<bool>(yaml_node, "enable_cspg_stats");
